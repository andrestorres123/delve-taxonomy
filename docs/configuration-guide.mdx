...
|-------|-----------|----------|
| `anthropic/claude-opus-4-8` | Highest reasoning capability | Default choice, most use cases |
| `anthropic/claude-sonnet-4-6` | Strong balance of capability and cost | Cost-sensitive runs on large corpora |
| `anthropic/claude-haiku-4-5` | Fast and cheap | Quick iterations, simple data |
...
```python
# Highest-quality taxonomy for complex data (default)
delve = Delve(model="anthropic/claude-opus-4-8")

# Cost-sensitive runs on large corpora
delve = Delve(model="anthropic/claude-sonnet-4-6")

# Quick iteration during development
delve = Delve(model="anthropic/claude-haiku-4-5")
```

<Tip>
Start with Claude Opus (the default) for the best category distinctions. If cost matters more than nuance on a large corpus, drop down to `anthropic/claude-sonnet-4-6` — the quality difference is often subtle for straightforward categorization tasks.
</Tip>
...
delve = Delve(fast_llm="anthropic/claude-opus-4-8")
```
...
**What it controls:** Which scikit-learn model is trained on the embeddings to scale labeling beyond the LLM-labeled sample. Only relevant when a classifier is trained (`0 < sample_size < total documents`).

**Available options:**

| Value | Model | Notes |
|-------|-------|-------|
| `"logistic"` (default) | `LogisticRegression` | Higher accuracy on dense embeddings and faster to train/predict |
| `"random_forest"` | `RandomForestClassifier` | The pre-0.2.1 default, kept for backwards compatibility |

```python
# LogisticRegression (default) - recommended
delve = Delve(sample_size=150, classifier="logistic")

# RandomForest - legacy behavior
delve = Delve(sample_size=150, classifier="random_forest")
```

<Info>
Both classifiers expose `predict_proba`, so `classifier_confidence_threshold` and `low_confidence_action` work the same way regardless of which you choose. LogisticRegression is the default because it generally beats RandomForest on high-dimensional embedding vectors.
</Info>
...
- The trained classifier (LogisticRegression by default)
- Category mappings
...
See the [Classifier Export & Training](/classifier-workflow) guide for complete documentation.
